    ...
    def get_action(self, episode_num, state):
        ...
        if random.random() < prob_random_action:
            ...
            q_vals = tf.concat(
                [
                    self.q_models[a](
                        np.asarray(self.env.get_afterstate(state, a)[0])[np.newaxis]
                    )
            ...
            if episode_num >= self.params["learning_starts"]:
                ...
                for a in range(self.env.action_space.n):
                    if any(actions.squeeze() == a):
                        ...
                        afterstates = np.asarray(
                            [self.env.get_afterstate(st, a)[0] for st in s]
                        ).reshape(s.shape)
                        ...
                        with tf.GradientTape() as tape:
                            ...
                            for n_a in range(self.env.action_space.n):
                                next_as = np.asarray(
                                    [self.env.get_afterstate(st, n_a)[0] for st in s_p]
                                ).reshape(s_p.shape)
